...
yue .obj = function ( name ) {
	...
	//本地数据
	obj .getlocal = yue .pipe .into ( function ( name ) {
		let data 
		let state
	...
	//set
	obj .setlocal = function ( name , data ) {
...
yue .load = function ( id , src ) {
	...
	.true ( function ( d , s ) {
		...
		if ( s == 404 ) {
			//alert(d)
			let D = `<img src="https://iconfont.alicdn.com/t/12362e43-a0f2-4f3a-a4da-26a89e720e85.png" style="width:100%" @iconfont/><br/><div style="text-align:center;font-size:25px;color:rgba(36,142,73)" onclick="location.href='index.html'">页面丢失了?返回首页</div>`
			yue .loaddata [ src ] = D
			id .html ( D )
			yue .close_wait ( )
			return
...
yue .wave .install = function ( conf ) {
	...
	$ ( dom ) .get ( ) .forEach ( s => {
		...
		let state = "close"
		let max = Math .sqrt ( w * w + h * h * 0.618 )
		let off = s .offset ( )
		let dom
		,l
		,t
			...
			//		alert("ids"+obj)
				...
				if ( obj ._type_ == "yue_pipe_into" ) {
					...
					ret [ j ] = yields ( function ( json ) {
						yue .pipe (
							{
								_type_: last .yield == true ? "yue_pipe_yield_exe" : "null",
								data: last .data
							},
							json,
							yue .pipe .last ( { ret } )
